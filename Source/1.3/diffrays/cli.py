...
def run_diff_safe(old_path, new_path, output_db, log_file, debug_mode):
    ...
    try:
        ...
        if debug_mode:
            ...
            log.info(f"Total analysis time: {elapsed_time:.2f} seconds")
        
        # Display function count summary
        try:
            import sqlite3
            conn = sqlite3.connect(output_db)
            cursor = conn.cursor()
            
            # Get function counts from function_diffs table
            cursor.execute("SELECT COUNT(*) FROM function_diffs")
            total_functions = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM function_diffs WHERE old_pseudocode IS NOT NULL AND new_pseudocode IS NOT NULL")
            matched_functions = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM function_diffs WHERE old_pseudocode IS NULL AND new_pseudocode IS NOT NULL")
            added_functions = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM function_diffs WHERE old_pseudocode IS NOT NULL AND new_pseudocode IS NULL")
            removed_functions = cursor.fetchone()[0]
            ...
            print(f"  Added functions: {added_functions}")
            print(f"  Removed functions: {removed_functions}")
            
        except Exception as e:
            if debug_mode:
                log.warning(f"Could not retrieve function counts: {e}")
            print(f"\nAnalysis complete! Database: {output_db}")
            print(f"Elapsed time: {elapsed_time:.2f} seconds")
