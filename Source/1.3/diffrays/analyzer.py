...
# Suppress MCP plugin messages
os.environ['IDA_SUPPRESS_MCP'] = '1'
os.environ['IDA_QUIET'] = '1'

# Redirect stderr temporarily to suppress MCP messages
import contextlib
import io
    ...
    try:
        ...
        for name in all_names:
            ...
            if old_pseudo and new_pseudo:
                ...
                old_blocks = old_data.get('blocks') if old_data else None
                new_blocks = new_data.get('blocks') if new_data else None
                
                if old_blocks is not None and new_blocks is not None:
                    # Calculate block-based metrics
                    delta_blocks = abs(old_blocks - new_blocks)
                    block_ratio = min(old_blocks, new_blocks) / max(old_blocks, new_blocks) if max(old_blocks, new_blocks) > 0 else 0
                    ...
                    if delta_blocks == 0:
                        ...
                        s_ratio = (1 - similarity) * 0.2
                    else:
                        # Structural change -> emphasize block differences
                        s_ratio = (1 - similarity) * 0.6 + (1 - block_ratio) * 0.4
                else:
                    # Fall back to regular ratio if no block counts
                    s_ratio = ratio
            else:
